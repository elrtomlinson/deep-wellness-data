...
import { useMedicationEffectiveness, MedEffectivenessResult } from '@/hooks/useMedicationEffectiveness';
import { Pill, TrendingDown, TrendingUp, Minus, ChevronDown, ChevronUp, BarChart3 } from 'lucide-react';
import { cn } from '@/lib/utils';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Cell, CartesianGrid } from 'recharts';
import { format } from 'date-fns';

function RatingBadge({ rating }: { rating: MedEffectivenessResult['rating'] }) {
  const config = {
...
function MedCard({ result }: { result: MedEffectivenessResult }) {
  const [expanded, setExpanded] = useState(false);

  const comparisonData = result.avgSeverityOff > 0
    ? [
        { label: 'Taking', severity: result.avgSeverityOn, fill: 'hsl(var(--severity-low))' },
        { label: 'Not taking', severity: result.avgSeverityOff, fill: 'hsl(var(--severity-high))' },
      ]
    : null;
    ...
    <Card className="p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h4 className="font-medium text-sm">{result.medicationName}</h4>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-muted/50 rounded-lg p-2">
          <p className="text-lg font-bold tabular-nums">{result.daysTaken}</p>
          <p className="text-[10px] text-muted-foreground">Days taken</p>
        </div>
        <div className="bg-muted/50 rounded-lg p-2">
        ...
        <div className="h-24">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={comparisonData} layout="vertical" barSize={16}>
              ...
              <YAxis type="category" dataKey="label" tick={{ fontSize: 11 }} stroke="hsl(var(--muted-foreground))" width={70} />
              <Tooltip
                contentStyle={{
...
export function MedicationEffectivenessTracker() {
  const { logs, medications, symptoms } = useHealthData();
  const results = useMedicationEffectiveness(logs, medications, symptoms);
  ...
  return (
    <Card className="p-5 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-sm">Medication Effectiveness</h3>
      </div>

      <p className="text-xs text-muted-foreground">
        Compares average symptom severity on days you take each medication vs days you don't.
      </p>

      <div className="space-y-3">
        {results.map(r => (
          <MedCard key={r.medicationId} result={r} />
        ))}
      </div>
